...
def get_current_inventory_admin(token_data: TokenData = Depends(get_current_admin)) -> TokenData:
    allowed_inv_roles = ("admin", "super_admin", "inventory_admin",
                         "SUPER_ADMIN", "INVENTORY_ADMIN")
    if token_data.role not in allowed_inv_roles:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to access SIM Inventory resources"
        )
    return token_data
